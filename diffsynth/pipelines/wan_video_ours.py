...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        inputs_shared = {
            ...
            "prompt_text": prompt, "rope_dynamic": retro, "speed_schedule": retro_schedule, "speed_fast": s_edge, "speed_slow": s_mid, "speed_smooth": retro_smooth,
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Inference
            noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep, bias_step_idx=progress_id, bias_num_steps=num_inference_steps, beta_end=beta_end, beta_mid=beta_mid, bias_eps=5e-4, bias_apply_text=True, bias_apply_image=True)
            if cfg_scale != 1.0:
                if cfg_merge:
                    noise_pred_posi, noise_pred_nega = noise_pred_posi.chunk(2, dim=0)
                else:
                    # noise_pred_nega = self.model_fn(**models, **inputs_shared, **inputs_nega, timestep=timestep)
                    noise_pred_nega = self.model_fn(**models, **inputs_shared, **inputs_nega, timestep=timestep, bias_step_idx=progress_id, bias_num_steps=num_inference_steps, bias_apply_text=False, bias_apply_image=False)
...
def build_speed_vector(f, schedule, fast=1.06, slow=0.94, smooth=2, device="cpu"):
    dtype = torch.float32
    s = torch.ones(f, dtype=dtype, device=device)
    if schedule is None or len(schedule) == 0:
        return s
    t = torch.linspace(0.0, 1.0, steps=f, device=device, dtype=dtype)
    for label, a, b in schedule:
        a = float(a); b = float(b)
        if b < a: a, b = b, a
        a = max(0.0, min(1.0, a))
        b = max(0.0, min(1.0, b))
        mask = (t >= a) & (t < b if b < 1.0 else t <= b)
        if label.lower() == "fast":
            s[mask] = fast
    ...
    if smooth and smooth > 0:
        k = int(smooth)
        s_pad = torch.nn.functional.pad(s[None, None, :], (k, k), mode="replicate")
        w = torch.ones(1, 1, 2*k+1, device=device, dtype=dtype) / (2*k+1)
        s = torch.nn.functional.conv1d(s_pad, w)[0, 0]
    return s
    ...
    def process(self, pipe: WanVideoPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
        ...
        if end_image is not None:
            ...
            if pipe.dit.has_image_pos_emb:
                print("pipe.dit.has_image_pos_emb is True")
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    frame_tokens = int(h * w)

    if reference_latents is not None:
        f_eff = int(f - 1) 
        prefix_tokens = frame_tokens 
    else:
        f_eff = int(f)
        prefix_tokens = 0

    eps  = float(kwargs.get("bias_eps", 5e-4))
    step_idx   = kwargs.get("bias_step_idx", None)
    num_steps  = kwargs.get("bias_num_steps", None)
    beta_mid   = float(kwargs.get("beta_mid", 0.3))
    beta_end   = float(kwargs.get("beta_end", 0.7))
    apply_text = bool(kwargs.get("bias_apply_text", True))
    apply_image= bool(kwargs.get("bias_apply_image", True))

    _bias_cfg_common = {
        "f": f_eff, "h": int(h), "w": int(w),
    ...
    Df = dit.freqs[0].shape[-1]  # frame-axis
    Dh = dit.freqs[1].shape[-1]  # height-axis
    Dw = dit.freqs[2].shape[-1]  # width-axis

    rope_dynamic   = kwargs.get("rope_dynamic", False)
    speed_schedule = kwargs.get("speed_schedule", None)
    speed_fast     = float(kwargs.get("speed_fast", 1.06))
    speed_slow     = float(kwargs.get("speed_slow", 0.94))
    speed_smooth   = int(kwargs.get("speed_smooth", 2))

    freqs_dev = dit.freqs[0].device

    if rope_dynamic and speed_schedule:
        s_vec = build_speed_vector(
            f, speed_schedule,
            fast=speed_fast, slow=speed_slow, smooth=speed_smooth,
            device=freqs_dev,
        )
    else:
        s_vec = None
    ...
    if s_vec is None:
        frame_freqs = dit.freqs[0][:f]
    else:
        frame_freqs = dit.freqs[0][:f] * s_vec.view(f, 1)

    freqs = torch.cat([
        frame_freqs.view(f, 1, 1, Df).expand(f, h, w, Df),
        dit.freqs[1][:h].view(1, h, 1, Dh).expand(f, h, w, Dh),
        dit.freqs[2][:w].view(1, 1, w, Dw).expand(f, h, w, Dw),
    ], dim=-1).reshape(f * h * w, 1, Df + Dh + Dw).to(x.device)         
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            block.cross_attn._bias_cfg = None
            
            if 0 < step_idx / num_steps <= 0.4:
                if L_START <= block_id <= L_END:
                    # print(step_idx, block_id)
                    cfg_this = dict(_bias_cfg_common)
                    cfg_this["layer_id"] = int(block_id)
                    block.cross_attn._bias_cfg = cfg_this
